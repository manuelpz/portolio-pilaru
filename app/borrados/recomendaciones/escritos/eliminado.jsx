...
export default function Eliminado() {
    ...
    const URL_ESCRITOS = 'https://portfolio-back-dev-pkbc.1.us-1.fl0.io/api/escritos'

    useEffect(() => {
        setIsLoading(true)
        const fetchData = async () => {
            const res = await fetch(URL_ESCRITOS)
    ...
    const eliminarElemento = async (id) => {
        setIsLoading(true)
        try {
            await fetch(URL_DOCUMENTALES + `/${id}`, {
                method: 'DELETE'
            })
                ...
                return (
                    <div key={index} className="grid grid-cols-3 justify-items-center items-center border-blue-200 border-b-4 pb-6 mt-6">
                        <Image
                            ...
                            width={100}
                            height={100}
                            alt="Imagen de la recomendación a borrar" />
                        <p>{item.titulo}</p>
                        <Image
                        ...
                        >
                            <div className="flex flex-col justify-center items-center">
                                <p className="text-center">¿Estás seguro de que quieres eliminar esta recomendación?</p>
